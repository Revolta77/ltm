...
class TransXEditable extends React.Component {
    ...
    componentDidMount() {
        ...
        this.$el.revolta77_editable(defaults);
    ...
    componentWillUnmount() {
        this.$el.revolta77_editable('destroy');
    }
    ...
    static transXEditLink(group, key, locale, $t, $withDiff) {
        ...
        if ($t) {
            ...
            return (
                <span>
            <TransXEditable
                className={"revolta77_editable status-" + ($t.status || 0) + ' locale-' + $t.locale/* + ($t.value === null ? ' editable-empty':'')*/}
            ...
            return (
                <span>
            <TransXEditable
                className={"revolta77_editable editable-empty status-" + (0) + ' locale-' + locale}
                // data-placement='bottom'
                data-type="textarea"
                data-pk={0}
                data-url={url}
