    ...
    public function inPlaceEditLink($t, $withDiff = false, $key = null, $locale = null, $useDB = null, $group = null)
    {
        try {
            ...
            if ($t) {
                ...
                $action = URL::action(ManagerServiceProvider::CONTROLLER_PREFIX . 'Revolta77\TranslationManager\Controller@postEdit', array($t->group));

                $result = '<a href="#edit" class="revolta77_editable status-' . ($t->status ?: 0)
                    . ' locale-' . $t->locale
...
HTML;
    ...
    {
        $config = App::get('config')[\Revolta77\TranslationManager\ManagerServiceProvider::PACKAGE];
        $key = \Revolta77\TranslationManager\Manager::DISABLE_REACT_UI;
